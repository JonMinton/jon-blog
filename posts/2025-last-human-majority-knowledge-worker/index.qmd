...
{{< include clock_animated.html >}}

*And this one? It shows the current UK time, updating every second. Same Claude, same session, asked to go further: "produce an animated analogue clock that shows the current time in the UK (always)." The approach: SVG graphics plus JavaScript that queries your browser's clock, converts to Europe/London timezone, calculates hand angles trigonometrically, and applies rotation transforms. The reasoning process—documented in full before writing any code—took longer than the implementation.*
