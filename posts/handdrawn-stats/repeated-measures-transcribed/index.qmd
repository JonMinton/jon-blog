...
![Slopes for arms A and B by sign of $\beta_B$, and a note on nested models and the F-test](img_10_m5_green_box.png)

This kind of model spec helps for comparing the slopes, but now we've neglected the fact of observations being grouped in subjects. *[Green annotation in original: & subjects have different initial weights]*
...
$$Y_i \sim f(\theta_i, \alpha) \qquad \text{← Stochastic Component}$$

$$\theta = g(X_i, \beta) \qquad \text{← Systematic Component}$$

*[Green note in original: This is a very generalised & abstracted notation. The $\alpha$ & $\beta$ here mean something different to the $\alpha$ & $\beta$ used elsewhere in this post.]*
...
Overall variation ($\epsilon$) = group-level variation ($\tau$) + residual/remaining variation ($e$)
...
### Addendum: verification pass, July 2026

*This section is written by Claude (Fable 5), which revisited the source image five months after the original transcription — exactly the experiment the reflection above proposed.*

The original 73,000-pixel scan was re-sliced into 27 overlapping native-resolution segments and every segment read against this page. The handwriting that was at the edge of legibility in February 2026 — including the final section where the original attempt hallucinated content — now reads reliably, and the full pass surfaced and corrected around twenty transcription errors. Representative examples:

- "consider **sex** as a control variable" had become "age" (the dataset has a Sex column and no Age column);
- "we can perform an **F-test**" had become "Likelihood Ratio Tests", and the following sentence about falling back to AIC/BIC was missing entirely;
- the $M_{Full}$ / $M_{Rest}$ nested-model equations had been dropped;
- "a separate **n-of-1** 'study'" had become "n=2", and the clause "it's not something I've seen before" was missing;
- the closing sentence of the testing section was an invention not present in the original, now replaced with what the notes actually say;
- the heading "An 'Interesting' Detour" and the Grandmother Model passage (with the general $Y_i \sim f(\theta_i, \alpha)$, $\theta = g(X_i, \beta)$ notation and its green caveat note) were restored;
- several smaller word-level drifts ("Subject-Dates" → "Subject-Time", "facets" → "panels", "used" → "good", swapped symbols in the variance decomposition) were corrected.

The hand-drawn diagrams, coloured annotated equations, and sketches remain preserved as native-resolution crops of the original — that decision has aged well. What changed is that the surrounding prose can now be verified word-by-word against the pixels rather than reconstructed from them.
