...
And the pattern isn't just mine. Someone on Reddit posted a side-by-side from [MineBench](https://minebench.ai) - a benchmark-cum-toy which has models build Minecraft-style voxel scenes from a text prompt, outputting raw block coordinates, with humans voting pairwise on the results - giving both siblings the same two words: *a fighter jet*.
...
The prompt is singular, and Fable's reading of it is exact: one jet, flying low over a runway, with just enough scenery to frame it - 83,457 blocks. Opus built 512,910 blocks - six times as many - and *three* fighter jets, of different sizes, banking over an ocean through launch plumes and towering clouds. Nobody asked for the second jet, let alone the third. Nor is this a one-off: the thread carries about half a dozen such comparisons, and the pattern holds across all of them. Opus overengineers, filling the voxel space as if to impress; Fable is consistently more restrained - subtle and precise about the brief, down to respecting the difference between 'a fighter jet' and 'fighter jets'. And because MineBench's output *is* the blocks, the swot's thoroughness here is literally made of output tokens: the token tax, rendered in voxels. (Which scene is *better* is exactly what the benchmark settles by human vote - and honestly, Opus's is more cinematic. The point isn't quality. It's disposition: the swot answers everything the prompt might conceivably have meant; the savant decides what it did mean, and stops.)

> **\[TODO - Gopher\]** Source link needed: the GIF's filename carries the Reddit slug ("differences-between-fable-5-and-opus-5-on..."), but Reddit is unreachable from this session and the thread isn't search-indexed yet. Paste the permalink here and credit the poster's username.
