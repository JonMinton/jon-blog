...
![MineBench, same prompt to both models. Top: Claude Fable 5's build - one jet on a runway, 83,457 blocks. Bottom: Claude Opus 5's - an aerial battle over an ocean, 512,910 blocks.](differences-between-fable-5-and-opus-5-on-v0-wva19c9dfnfh1.gif){#fig-minebench}

Fable built one jet, parked on a runway, with just enough scenery to frame it: 83,457 blocks. Opus built 512,910 blocks - six times as many - of jets banking over an ocean through launch plumes and towering clouds. Same prompt, same corporate parent, similar competence; utterly different ideas about how much work two words deserved. And because MineBench's output *is* the blocks, the swot's thoroughness here is literally made of output tokens: the token tax, rendered in voxels. (Which scene is *better* is exactly what the benchmark settles by human vote - and honestly, Opus's is more cinematic. The point isn't quality. It's disposition: the swot answers everything the prompt might conceivably have meant; the savant decides what it did mean, and stops.)
