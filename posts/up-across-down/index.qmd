...
> B: Very good.
> A: ...in Fortran.
> B: Then you didn't program it; you got the Fortran compiler to program it.

Generalise that dialogue across the decades and it becomes a scheme: the same conversation, restaged every ten or fifteen years, with yesterday's 'not real programming' promoted - without ceremony, and without apology - to today's 'real programming'.

::: {.column-page}
```{mermaid}
flowchart LR
  classDef real fill:#2e6b4f,color:#ffffff,stroke:#1d4733
  classDef fake fill:#8f3b3b,color:#ffffff,stroke:#5e2222
  classDef old fill:#555555,color:#eeeeee,stroke:#333333

  subgraph D40["1940s"]
    direction TB
    m40["Machine code<br/>'real programming'"]:::real
  end

  subgraph D50["1950s"]
    direction TB
    f50["Fortran<br/>'you got the compiler to do it'"]:::fake --> a50["Assembly<br/>'real programming' (now)"]:::real --> m50["Machine code"]:::old
  end

  subgraph D70["1970s"]
    direction TB
    c70["C<br/>'high-level languages waste the machine'"]:::fake --> f70["Fortran<br/>'real programming' (now)"]:::real --> r70["⋮"]:::old
  end

  subgraph D80["1980s"]
    direction TB
    b80["BASIC & construction kits<br/>'toy language, toy tools'"]:::fake --> c80["C<br/>'real programming' (now)"]:::real --> r80["⋮"]:::old
  end

  subgraph D90["1990s"]
    direction TB
    j90["Java, Visual Basic<br/>'the machine manages your memory for you'"]:::fake --> c90["C++<br/>'real programming' (now)"]:::real --> r90["⋮"]:::old
  end

  subgraph D00["2000s"]
    direction TB
    p00["Python, JavaScript<br/>'that's just scripting'"]:::fake --> j00["Java, C#<br/>'real programming' (now)"]:::real --> r00["⋮"]:::old
  end

  subgraph D10["2010s"]
    direction TB
    n10["No-code platforms, level editors, Scratch<br/>'that's just configuration'"]:::fake --> p10["Python<br/>'real programming' (now)"]:::real --> r10["⋮"]:::old
  end

  subgraph D20["2020s"]
    direction TB
    e20["English, via Claude<br/>'you just asked'"]:::fake --> n20["Everything below<br/>'real programming' (now)"]:::real --> r20["⋮"]:::old
  end

  D40 --> D50 --> D70 --> D80 --> D90 --> D00 --> D10 --> D20
```
:::

The boundary has never stopped moving, and it has only ever moved in one direction. Nobody now demands that you manage your own registers before being taken seriously. The Fortran objection didn't lose the argument; it died of old age, and its children took up the family business one rung higher.

And as with 'real programming', so with 'made by AI': the declaration being asked for assumes a line - work that is AI, work that is not - drawn cleanly enough for a moderator to enforce. It can't be. Nothing is ever fully AI or fully human. There is always some mixture, and the range of possible mixtures is very wide: AI engagement with human content, human engagement with AI content, and every ratio in between. I'm still learning where the right balance sits, and so is everyone else. [Cognitive centaurs](../guano-guild-as-cognitive-centaur-experiment/index.qmd).
