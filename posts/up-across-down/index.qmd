...
Usually, but not always, this happened around T3; motion became jerky, music became more stoccato, less fluid. The whole experience started to become more like wading through sand than ambling purposefully through a magic forest. Something was off.
...
The face the models weren't blind to each other's reviewers - something I should have addressed by using different branches for each, then merging after, meant later models *were* influenced the reviews left by earlier models. GPT went first, and seemed very thorough. Gemini went second, 'peeking' into GPT's review notes, and judging it to be better and more thorough than its own.
...
Here's an illustration, from the first of 15 such comparisons on a subreddit, showing how Fable 5 and Opus 5 differ when both prompted to produce 'a fighter jet' in minebench.ai:

And the pattern isn't just mine. [A post on r/ClaudeAI](https://www.reddit.com/r/ClaudeAI/s/kUK3YEsuJu) collects fifteen side-by-side comparisons from [MineBench](https://minebench.ai) - a benchmark-cum-toy which has models build Minecraft-style voxel scenes from a text prompt, outputting raw block coordinates, with humans voting pairwise on the results. Here is what the two siblings did with the same three words, *a fighter jet*:

![MineBench, same prompt to both models. Top: Claude Fable 5 - a single jet flying low over a runway, 83,457 blocks. Bottom: Claude Opus 5 - three jets of different sizes in an aerial battle over an ocean, 512,910 blocks.](differences-between-fable-5-and-opus-5-on-v0-wva19c9dfnfh1.gif){#fig-minebench alt="MineBench, same prompt to both models. Top: Claude Fable 5 - a single jet flying low over a runway, 83,457 blocks. Bottom: Claude Opus 5 - three jets of different sizes in an aerial battle over an ocean, 512,910 blocks."}

The prompt is singular, and Fable's reading of it is exact: one jet, flying low over a runway, with just enough scenery to frame it - 83,457 blocks. Opus built 512,910 blocks - six times as many - and *three* fighter jets, of different sizes, banking over an ocean through launch plumes and towering clouds. Nobody asked for the second jet, let alone the third. Nor is this a one-off: the post carries fifteen such comparisons, and the pattern holds across them. Opus overengineers, filling the voxel space as if to impress; Fable is consistently more restrained - subtle and precise about the brief, down to respecting the difference between 'a fighter jet' and 'fighter jets'. And because MineBench's output *is* the blocks, the swot's thoroughness here is literally made of output tokens: the token tax, rendered in voxels. (Which scene is *better* is exactly what the benchmark settles by human vote - and honestly, Opus's is more cinematic. The point isn't quality. It's disposition: the swot answers everything the prompt might conceivably have meant; the savant decides what it did mean, and stops.) \## Why Fable *shouldn't* be more swottish

**Savant behaviour:**

- High reliance on reasoning from first principles
- Creative, and often more intelligent, inference about the latent subgoals and implicit assumptions behind a prompt
