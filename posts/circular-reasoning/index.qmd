...
## A Note from Claude (added July 2026)

*Jon asked me to evaluate this post's argument, then to park the undeveloped ideas here rather than lose them in a chat thread. Three, briefly:*

**The critique is fair for a specific reason worth naming.** The usual defence - "it's just a metaphor, you're being pedantic about the vehicle" - fails here because the metaphor was load-bearing: its entire purpose was to derive a blame allocation ("the liberals are at least as much to blame, so we hate them both") from the mechanics of motor, brake and teeth. Once a metaphor's mechanism *is* the argument, you are entitled to inspect the machine. And on Jon's reconstruction, the circular topology was the original authors' own: they mapped the full circle, with far left and far right adjacent at the bottom. The horseshoe entailment was already drawn; this post just traced the arrow. (The argument's one exposure: everything hinges on the fidelity of "something as follows".)

**The post stops one inference early.** The model contains no leftward motor at all - a rightward motor, a brake, and nothing else. Within its own mechanics, the wheel fails to move left not because liberals brake it but because nothing is pulling it. The brake only ever engages because nothing else is happening. A model offered as an indictment of the *faux* left quietly represents the *actual* left as motorless - a deeper self-own than the horseshoe adjacency.

**And a grimmer corollary.** Since the ratchet's teeth block anticlockwise motion, the only route back to the left this model permits is clockwise - all the way round, through the far right. Radicalisation flips are not merely possible in the model; they are its sole available path to the destination its authors want. (For what it's worth, the implementation above is faithful to the mechanism: the sawtooth, traversed clockwise, is exactly "jut up vertically, then fall back linearly" - the smooth faces run rightward, the vertical faces catch the brake. The machine was built as specified before being disagreed with.)
