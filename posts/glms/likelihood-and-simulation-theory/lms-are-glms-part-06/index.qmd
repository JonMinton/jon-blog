...
In [the next part of this series](../lms-are-glms-part-07/index.qmd), we'll see how other outputs available from `optim` can be used to estimate uncertainty in the parameters of interest, how this information can be used to produce the kinds of estimates of standard errors around coefficients which are summarised in `glm` and `lm` `summary()` functions, and which many (ab)users of statistical models obsess about when star-gazing, and how information about uncertainty in parameter estimates allows for more honest model-based predictions.[^claude-optimizers]
